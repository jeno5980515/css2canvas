...
_html2canvas.Parse = function (images, options, cb) {
  ...
  function getTransform(element, parentStack) {
    var transformRegExp = /(matrix|matrix3d)\((.+)\)/;
  ...
  function matrixInverse(m) {
    var m_parse = m.replace('matrix3d(','').replace(')','').split(',').map(parseFloat) ;
    if ( isNaN(m_parse[0]) ) return [] ;
    return [m_parse[0],m_parse[1],m_parse[4],m_parse[5],m_parse[12],m_parse[13]];
  ...
  function renderElement(element, parentStack, ignoreBackground) {
    ...
    // var transform = matrixInverse(getCSS(element, "transform")) ;
    // if ( transform ){
    //   ctx.setVariable('transform',1, 0.5, -0.5, 0.5, 30, 10);
    // }
